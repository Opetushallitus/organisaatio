describe('Organisaatiot Page', () => {
  ...
  it('Can save edited organisation', () => {
    ...
    cy.get('#react-select-3-input').type('Hakukohde{enter}{enter}', {force: true});
    cy.get('#react-select-5-input').type('Yleinen{enter}{enter}', {force: true});
    cy.intercept('PUT', '/organisaatio/organisaatio/v4/1.2.246.562.10.48587687889', { fixture: 'humakRyhma.json' });
    cy.get('button').contains('TALLENNA').click();
    cy.get('div#root').children().first().should('have.attr', 'color', 'primary');
  })

        ...
        },
        {
            ...
            Cell: ({ row }: any) => {
                if (row.original.tyypit.length > 0) {
                    return row.original.tyypit
                        .map((tyyppi: string) => {
                            const koodityyppi = ryhmaTyypitKoodisto
                                .koodit()
                                .find((rt) => rt.uri === tyyppi.slice(0, -2));
                            if (koodityyppi) {
                                return (
                                    koodityyppi.nimi[language] ||
                                    koodityyppi.nimi['fi'] ||
                                    koodityyppi.nimi['sv'] ||
                                    koodityyppi.nimi['en']
                                );
            ...
            Cell: ({ row }: any) => {
                if (row.original.kayttoryhmat.length > 0) {
                    return row.original.kayttoryhmat
                        .map((tyyppi: string) => {
                            const koodiRyhma = kayttoRyhmatKoodisto
                                .koodit()
                                .find((rt) => rt.uri === tyyppi.slice(0, -2));
                            if (koodiRyhma) {
                                return (
                                    koodiRyhma.nimi[language] ||
                                    koodiRyhma.nimi['fi'] ||
                                    koodiRyhma.nimi['sv'] ||
                                    koodiRyhma.nimi['en']
                                );
        ...
        <TyypitJaRyhmatKehys>
            ...
            <div className={styles.FiltteriContainer}>
                <div className={styles.FiltteriRivi}>
                    ...
                    <Button variant="outlined" style={{ borderRadius: '100%', height: '2rem', width: '2rem' }}>
                        ?
                    </Button>
                </div>
                <div className={styles.FiltteriRivi}>

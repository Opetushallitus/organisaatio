...
public class EntityToVapaatKuvauksetFunction implements Function<OrganisaatioMetaData, List<KuvailevaTietoTyyppi>> {
    ...
    private final EntityToMonikielinenTekstiTyyppiFunction toMonikielinenTekstiTyyppi = new EntityToMonikielinenTekstiTyyppiFunction();

    @Override
    public List<KuvailevaTietoTyyppi> apply(@Nullable final OrganisaatioMetaData metadata) {
        final List<KuvailevaTietoTyyppi> kuvailevat = Lists.newArrayList();
        ...
        for (KuvailevaTietoTyyppiTyyppi tyyppi : KuvailevaTietoTyyppiTyyppi.values()) {
            ...
            if (value != null) {
                final KuvailevaTietoTyyppi dto = new KuvailevaTietoTyyppi();
                dto.setTyyppi(tyyppi);
                dto.setSisalto(toMonikielinenTekstiTyyppi.apply(value));
                kuvailevat.add(dto);
            }
        }
        return kuvailevat;

...
public class OrganisaatioRepositoryImpl implements OrganisaatioRepositoryCustom {
    ...
    public Collection<Organisaatio> findBy(SearchCriteria criteria, Date now) {
        ...
        JPAQuery<OrganisaatioPerustietoRivi> query = new JPAQuery<>(em)
                .from(qOrganisaatio)
        ...
        return query.fetch().stream()
                .collect(groupingBy(OrganisaatioPerustietoRivi::getOid,
                        reducing(new Organisaatio(), OrganisaatioRepositoryImpl::map, OrganisaatioRepositoryImpl::merge)))
                .values();
    }
    ...
    public boolean isYtunnusAvailable(String ytunnus) {
        return ((Number) em
                .createQuery("SELECT COUNT(*) FROM " + Organisaatio.class.getName() + " WHERE ytunnus=:ytunnus AND organisaatio_poistettu = FALSE")
    ...
    public List<String> findParentOidsTo(String oid) {
        ...
        while (org != null) {
            oids.add(org.getOid());
            OrganisaatioSuhde curSuhde = organisaatioSuhdeRepository.findParentTo(org.getId(), new Date());

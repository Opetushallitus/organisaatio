...
begin
    WITH
        ...
        yhteys AS (INSERT INTO yhteystieto (dtype, id, version,yhteystietooid, osoite, osoitetyyppi, postinumero, postitoimipaikka,email, organisaatio_id, kieli)
            ...
            from org,
                 (values ('Osoite',concat(oid1,'.1'),'FOOADDRESS','posti','posti_00009','DIGITOINTI',null,'kieli_fi#1',1),
                         ('Email' ,concat(oid1,'.2'),null,null,null,null,'test@test.fi','kieli_fi#1',2),
                         ('Osoite',concat(oid1,'.3'),'FOOADDRESS','kaynti','posti_00009','DIGITOINTI',null,'kieli_fi#1',3))as
                     osoite(type, oid, osoite, osoitetyyppi, postinumero, postitoimipaikka, email,kieli,number))

...
begin
    WITH
        ...
        vaka_tt AS (INSERT INTO varhaiskasvatuksen_toimipaikka_tiedot (id, version, toimintamuoto, kasvatusopillinen_jarjestelma, paikkojen_lukumaara)
                    SELECT nextval('public.hibernate_sequence'), 0, vardatoimintamuoto, 'vardakasvatusopillinenjarjestelma_kj98', 0 FROM checker WHERE vardatoimintamuoto IS NOT NULL RETURNING id),
        ...
        parent  AS (INSERT INTO organisaatio_parent_oids (organisaatio_id, parent_position, parent_oid) SELECT org.id, 0, parentoid from org where parentoid is not null),
...
with parent as (select id,oid,parentidpath from createOrganisaatio('1.2.246.562.99.00000000001','Mansikkalan testi kunta',null,0, '|0|', array['organisaatiotyyppi_01','organisaatiotyyppi_09'], null, null, null, null) as foo(id bigint,oid varchar, parentidpath varchar)),
...
with parent as (select id,oid,parentidpath from createOrganisaatio('1.2.246.562.99.00000000005','Testi Koulutuskuntayhtymä Puolukka',null,0, '|0|', array['organisaatiotyyppi_01','organisaatiotyyppi_09'], null, null, null, null) as foo(id bigint,oid varchar, parentidpath varchar)),
...
with parent as (select id,oid,parentidpath from createOrganisaatio('1.2.246.562.99.00000000008','Mustikkalan testi yhdistys',null,0, '|0|', array['organisaatiotyyppi_01','organisaatiotyyppi_05'], null, null, null, null) as foo(id bigint,oid varchar, parentidpath varchar))
...
with parent as (select id,oid,parentidpath from createOrganisaatio('1.2.246.562.99.00000000011','Varhaiskasvatuksen järjestäjä kunta testi',null,0, '|0|', array['organisaatiotyyppi_07'], null, null, null, null) as foo(id bigint,oid varchar, parentidpath varchar)),
...
with parent as (select id,oid,parentidpath from createOrganisaatio('1.2.246.562.99.00000000015','Varhaiskasvatuksen järjestäjä yksityinen',null,0, '|0|', array['organisaatiotyyppi_07'], null, null, 'Yksityinen elinkeinonharjoittaja', null) as foo(id bigint,oid varchar, parentidpath varchar)),

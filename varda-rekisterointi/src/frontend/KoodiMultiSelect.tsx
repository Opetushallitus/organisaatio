...
export default function KoodiMultiSelect(props: Props) {
    ...
    const options = props.selectable.map(koodi => ({value: koodi.uri,
                                                    label: toLocalizedText(koodi.nimi, language, koodi.arvo)}))
                                    .sort((option1, option2) => option1.label.localeCompare(option2.label, language));
    const defaultValue: OptionTypeBase[] = props.selected
        ? options.filter(option => props.selected
                            ? props.selected.some(select => select === option.value)
                            : [])
        : [];
    return (
        <div className="oph-input-container">
            <Select
                ...
                onChange={(selectionOptions) => props.onChange(selectionOptions

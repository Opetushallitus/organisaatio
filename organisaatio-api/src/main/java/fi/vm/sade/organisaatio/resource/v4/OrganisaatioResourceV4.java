...
public interface OrganisaatioResourceV4 {
    ...
    @GetMapping(path = "/{oid}", produces = MediaType.APPLICATION_JSON_VALUE + ";charset=UTF-8")
    @ApiOperation(
    ...
    @GetMapping(path = "/{oid}/jalkelaiset", produces = MediaType.APPLICATION_JSON_VALUE + ";charset=UTF-8")
    OrganisaatioHakutulosV4 findDescendants(@ApiParam(value = "Organisaation oid", required = true) @PathVariable String oid);

    @PutMapping(path = "/{oid}/organisaatiosuhde/{parentoid}", produces = MediaType.APPLICATION_JSON_VALUE + ";charset=UTF-8")
    @ApiOperation(
